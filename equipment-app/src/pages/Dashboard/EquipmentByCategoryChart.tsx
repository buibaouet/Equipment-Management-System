...
import { EquipmentByCategoryModel } from "../../types/Dashboard";

const CHART_COLORS = ["#3641f5", "#7592ff", "#dde9ff", "#b5c6ff", "#9aa9ff"];

export default function EquipmentByCategoryChart() {
  const { data, isFetching } = useGetDashboardDataQuery();
  const categoryData = useMemo(
    () => data?.data?.equipmentByCategory ?? [],
    [data?.data?.equipmentByCategory]
  );
  ...
  const totalEquipment = useMemo(
    () =>
      categoryData.reduce(
        (total: number, item: EquipmentByCategoryModel) => total + item.count,
        0
      ),
    [categoryData]
  );
      ...
      <div className="flex flex-col items-center gap-8 xl:flex-row">
        ...
        <div className="flex flex-col items-start gap-6 sm:flex-row xl:flex-col w-full max-h-100 overflow-y-auto pr-1">
          {categoryData.map((item, index) => {
            ...
            return (
              <div className="flex items-start gap-2.5" key={item.categoryId}>
                <div
          ...
          {!isFetching && categoryData.length === 0 && (
